...
def _get_ocr() -> Tuple[Optional[Any], Optional[str]]:
    """Initialize and return the OCR engine.
    
    Returns:
        Tuple of (ocr_instance, backend_name).
    """
    global _ocr_instance, _ocr_backend
    if _ocr_instance is not None:
        return _ocr_instance, _ocr_backend
    
    # Try RapidOCR first (fastest, ONNX-based)
    try:
...
class OCRAction(BaseAction):
    ...
    @classmethod
    def get_ocr(cls) -> Tuple[Optional[Any], Optional[str]]:
        """Get the OCR engine instance and backend name."""
    ...
    def _do_ocr(
        ...
        else:  # PaddleOCR (legacy fallback)
